...
bash ~/KINGELNET/.banner.sh ${col} ${TNAME}
echo "";
...
tput sgr 0
if [[ ${PROC32} == [Y/y] ]]; then
	if [ -e $HOME/KINGELNET.txt ]; then
		rm $HOME/KINGELNET.txt;
	fi

	if [ -d $HOME/KINGELNET ]; then
	cd $HOME/KINGELNET
	fi
...
clear;
echo -e "\033[31m$(Youssef - KING EL NET "KINGELNET")\e[0m"
echo -e "\e[1;32m
+----------------------------------*/
